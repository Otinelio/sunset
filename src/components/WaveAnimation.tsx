export function WaveAnimation({ className = "" }: { className?: string }) {
  return (
    <div className={`pointer-events-none w-full overflow-hidden ${className}`}>
      <svg viewBox="0 0 1440 80" preserveAspectRatio="none" className="w-[200%] wave-animate h-16 md:h-20">
        <defs>
          <linearGradient id="waveg" x1="0" x2="1">
            <stop offset="0%" stopColor="#E85C3A" stopOpacity="0.85" />
            <stop offset="100%" stopColor="#F2A65C" stopOpacity="0.85" />
          </linearGradient>
        </defs>
        <path
